...
import org.banksolution.infrastructure.tigerbeetle.TigerBeetleAccountRepository;
...
public class LedgerAccountService {
    ...
    public LedgerAccount createLedgerAccount(UUID accountId, Currency currency) {
        ...
        return tigerBeetleAccountRepository.persist(LedgerAccount.of(accountId, currency));
    }
    ...
    public List<LedgerAccount> createLedgerAccounts(List<LedgerAccount> ledgerAccounts) {
        ...
        return tigerBeetleAccountRepository.persistAll(ledgerAccounts);
    }

    public LedgerAccount getLedgerAccount(UUID ledgerAccountId) {
        log.info("Fetching ledger account: {}", ledgerAccountId);
        return tigerBeetleAccountRepository.findById(ledgerAccountId)
    ...
    public LedgerAccount getWallet(UUID accountId, Currency currency) {
        return getLedgerAccount(LedgerAccountIds.wallet(accountId, currency));
    }

    public List<LedgerAccount> getWallets(UUID accountId) {
        List<UUID> candidateIds = java.util.Arrays.stream(Currency.values())
                .map(currency -> LedgerAccountIds.wallet(accountId, currency))
                .toList();

        return tigerBeetleAccountRepository.findAll(candidateIds);
    }
}

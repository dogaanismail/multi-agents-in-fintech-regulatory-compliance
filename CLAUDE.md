...
- **Every `.avsc` is self-contained.** One file per event, with nested records and enums declared inline — never a
  separate file for an enum, and never a cross-file type reference. Reading one event file tells you the whole contract.
  Repeating the *same* named type inline across files is fine: `generateAvro` merges identical definitions into one
  generated class. It fails only when two definitions of one fully-qualified name **differ**, so when you add a symbol,
  add it everywhere the enum appears (`PostingInstructionType` lives in both ledger events; `PaymentScheme` in
  `PaymentCreatedEvent` and `PaymentSnapshotEvent`).
- **Prefer an Avro `enum` over `string`** for closed vocabularies, so the contract documents its own valid values.
  Domain ↔ Avro conversion is `valueOf(x.name())` in the mapper (`toAvroPostingInstructionType`, `mapPaymentScheme`);
  Axon commands and events keep the
  `String` form, because enums baked into a stored event stream are brittle to evolve.
